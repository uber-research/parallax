...
from bokeh.models import ColumnDataSource, HoverTool, Div, LabelSet, RangeSlider, MultiSelect, Line, Button, Segment
...
source_1 = ColumnDataSource(data=dict(x=[], y=[], color=[], label=[],  legend=[]))
source_2 = ColumnDataSource(data=dict(x=[], y=[], color=[], label=[], legend=[]))
...
scatter_1 = plot.circle(x="x", y="y", source=source_1, size=7, color="color", line_color=None, fill_alpha=0.6,
                        name='scatter_1',
                        legend='legend')
scatter_2 = plot.circle(x="x", y="y", source=source_2, size=7, color="color", line_color=None, fill_alpha=0.6,
                        name='scatter_2',
                        legend='legend')
segments = Segment(x0="x0", y0="y0", x1="x1", y1="y1", line_color="color", line_alpha=0.6, line_width=2,
                   line_dash='dashed',
                   name='scatter')
...
if args.labels:
    labels_annotations_1 = LabelSet(x="x", y="y", text="label", y_offset=1,
                                    ...
                                    source=source_1, text_align='center', text_alpha=1)
    plot.add_layout(labels_annotations_1)
    labels_annotations_2 = LabelSet(x="x", y="y", text="label", y_offset=1,
                                    text_font_size="8pt", text_color="color",
                                    source=source_2, text_align='center', text_alpha=1)
    plot.add_layout(labels_annotations_2)

# Create Input controls
dataset_1 = Select(title="Dataset #1", options=data_manager.dataset_ids, value=selected_dataset_1)
dataset_2 = Select(title="Dataset #2", options=data_manager.dataset_ids, value=selected_dataset_2)
...
metadata_filters_title_1 = Div(text="<strong>Metadata Filters Dataset 1</strong>")

rank_slice_1 = RangeSlider(title="Rank Slice", value=(1, data_manager.get_size(selected_dataset_1)), start=1,
                           end=data_manager.get_size(selected_dataset_1), step=1)
...
metadata_filters_title_2 = Div(text="<strong>Metadata Filters Dataset 2</strong>")

rank_slice_2 = RangeSlider(title="Rank Slice", value=(1, data_manager.get_size(selected_dataset_2)), start=1,
                           end=data_manager.get_size(selected_dataset_2), step=1)
...
def build_data_filter():
    ...
    dataset_cf = Select(title="Dataset", options=['Both', selected_dataset_1, selected_dataset_2], value='Both')
...
if args.labels:
    show_labels = Select(title="Show labels:", value="True", options=["True", "False"])
    ...
    if x_axis_value == '' or y_axis_value == '':
        ...
        for metadata_filter in metadata_filters_1:
            if metadata_type_1[metadata_filter.title] == 'boolean':
                if metadata_filter.value != 'Any':
                    metadata_filters_params_1.append((metadata_filter.title, metadata_filter.value == 'True'))
            elif metadata_type_1[metadata_filter.title] == 'numerical':
                filter_value = (int(metadata_filter.value[0]), int(metadata_filter.value[1]))
                if filter_value != (int(rank_slice_1.start), int(rank_slice_1.end)):
                    metadata_filters_params_1.append((metadata_filter.title, filter_value))
            elif metadata_type_1[metadata_filter.title] == 'categorical':
                filter_value = set(metadata_filter.value)
                if len(filter_value) > 0 and filter_value != metadata_domain_1[metadata_filter.title]:
                    metadata_filters_params_1.append((metadata_filter.title, filter_value))
        ...
        for metadata_filter in metadata_filters_2:
            if metadata_type_2[metadata_filter.title] == 'boolean':
                if metadata_filter.value != 'Any':
                    metadata_filters_params_2.append((metadata_filter.title, metadata_filter.value == 'True'))
            elif metadata_type_2[metadata_filter.title] == 'numerical':
                filter_value = (int(metadata_filter.value[0]), int(metadata_filter.value[1]))
                if filter_value != (int(rank_slice_2.start), int(rank_slice_2.end)):
                    metadata_filters_params_2.append((metadata_filter.title, filter_value))
                ...
                filter_value = set(metadata_filter.value)
                if len(filter_value) > 0 and filter_value != metadata_domain_2[metadata_filter.title]:
                    metadata_filters_params_2.append((metadata_filter.title, filter_value))
        ...
        rank_slice_values_1 = (int(rank_slice_1.value[0]), int(rank_slice_1.value[1]))
        if rank_slice_values_1 == (int(rank_slice_1.start), int(rank_slice_1.end)):
            rank_slice_values_1 = None

        rank_slice_values_2 = (int(rank_slice_2.value[0]), int(rank_slice_2.value[1]))
        if rank_slice_values_2 == (int(rank_slice_2.start), int(rank_slice_2.end)):
            rank_slice_values_2 = None

        embeddings_1 = projection(data_manager,
        ...
        embeddings_2 = projection(data_manager,
        ...
        for key in common_keys:
            ...
            slope = (embeddings_2[key]['coords'][1] -embeddings_1[key]['coords'][1]) / \
                    (embeddings_2[key]['coords'][0] - embeddings_1[key]['coords'][0])

            if distance >= difference_slider.value[0] and distance <= difference_slider.value[1] and \
                    slope >= slope_slider.value[0] and slope <= slope_slider.value[1] :
                embeddings[key] = {'coords_1': embeddings_1[key]['coords'], 'coords_2': embeddings_2[key]['coords']}
...
def update(attr, old, new):
    ...
    for label, emb_dict in embeddings.items():
        ...
        max_difference = max(max_difference, np.linalg.norm(emb_dict['coords_1'] - emb_dict['coords_2']))
        labels.append(label)
        colors.append(highlight_segment_color if label in highlight_items else default_segment_color)
        colors_1.append(highlight_color_1 if label in highlight_items else default_color_1)
        colors_2.append(highlight_color_2 if label in highlight_items else default_color_2)
...
def update_dataset(attr, old, new):
    ...
    for df_group in data_filters_groups:
        df_group[4].update(options=['Both', selected_dataset_1, selected_dataset_2])
        if df_group[4].value not in ['Both', selected_dataset_1, selected_dataset_2]:
...
def update_viz(attr, old, new):
    ...
    if args.labels:
        ...
        labels_annotations_1.text_font_size = str(int(labels_font_size.value)) + 'pt'
        labels_annotations_2.text_alpha = int(show_labels.value == 'True')
        labels_annotations_2.text_font_size = str(int(labels_font_size.value)) + 'pt'
...
def build_controls():
    controls = [dataset_1, dataset_2, measure, x_axis, y_axis, items,
                ...
                data_filters_title, difference_slider, slope_slider, *data_filters]
